...
void ImGui_ImplGlfw_MouseButtonCallback(GLFWwindow* window, int button, int action, int mods)
{
    ...
    if (action == GLFW_PRESS && button == GLFW_MOUSE_BUTTON_LEFT)
        g_MouseJustPressed[GLFW_MOUSE_BUTTON_LEFT] = true;
...
static void ImGui_ImplGlfw_UpdateGamepads()
{
    ...
    int axes_count = 0, buttons_count = 0, hats_count = 0;
    const float* axes = glfwGetJoystickAxes(GLFW_JOYSTICK_1, &axes_count);
    const unsigned char* buttons = glfwGetJoystickButtons(GLFW_JOYSTICK_1, &buttons_count);
    const unsigned char* hats = glfwGetJoystickHats(GLFW_JOYSTICK_1, &hats_count);
    ...
    // MAP_BUTTON(ImGuiNavInput_DpadUp,     12);    // D-Pad Up
    // MAP_BUTTON(ImGuiNavInput_DpadDown,   14);    // D-Pad Down
